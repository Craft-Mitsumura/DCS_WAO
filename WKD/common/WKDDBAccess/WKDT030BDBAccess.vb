...
Public Class WKDT030BDBAccess
    ...
    Public Function UpdateTInstructorFurikomi(pgid As String, simenengetsu As String, ownerno As String, Optional targetList As List(Of TNenchoEntity) = Nothing) As Boolean
        ...
        sql.AppendLine("    tainen = case when coalesce(tif.tainen, '') in ('', '0000') then lpad(extract(year from (date_trunc('month', to_date(substr(@simenengetsu, 1, 6), 'YYYYMM')) + interval '-1 day'))::text, 4, '0') else tif.tainen end")
        sql.AppendLine("  , taituki = case when coalesce(tif.taituki, '') in ('', '00') then lpad(extract(month from (date_trunc('month', to_date(substr(@simenengetsu, 1, 6), 'YYYYMM')) + interval '-1 day'))::text, 2, '0') else tif.taituki end")
        sql.AppendLine("  , taihi = case when coalesce(tif.taihi, '') in ('', '00') then lpad(extract(day from (date_trunc('month', to_date(substr(@simenengetsu, 1, 6), 'YYYYMM')) + interval '-1 day'))::text, 2, '0') else tif.taihi end")
