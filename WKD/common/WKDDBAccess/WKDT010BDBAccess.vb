...
Public Class WKDT010BDBAccess
    ...
    Public Function InsertTNencho(shoriNendo As String, pgid As String) As Boolean
        ...
        sql.AppendLine("select nen.sakuhyokbn, nen.dtnengetu, nen.itakuno, nen.bakyny, nen.instno, max(nen.frinengetu) max_frinengetu")
        ...
        sql.AppendLine("select nen.sakuhyokbn, nen.dtnengetu, nen.itakuno, nen.bakyny, nen.instno, max(nen.ownerno) max_ownerno")
        sql.AppendLine("from nen")
        sql.AppendLine("inner join nen2 on nen.sakuhyokbn = nen2.sakuhyokbn")
        ...
        sql.AppendLine("from nen")
        sql.AppendLine("inner join nen3 on nen.sakuhyokbn = nen3.sakuhyokbn")
        ...
        sql.AppendLine("from")
        sql.AppendLine("    nen")
        sql.AppendLine("inner join nen4 on nen4.sakuhyokbn = nen.sakuhyokbn")
    ...
    Public Function GetTNencho(shoriNendo As String, Optional targetList As List(Of TNenchoEntity) = Nothing) As DataTable
        ...
        sql.AppendLine("        when tainen is not null")
        sql.AppendLine("             and trim(tainen) <> ''")
        sql.AppendLine("             and tainen not in ('0','00','000','0000')")
        sql.AppendLine("        then '＊'")
        sql.AppendLine("        else ''")
        ...
        sql.AppendLine("              and trim(tainen) <> ''")
        sql.AppendLine("              and tainen not in ('0','00','000','0000')")
        sql.AppendLine("         then tainen || taituki || taihi")
        sql.AppendLine("        else ")
        sql.AppendLine("            case")
