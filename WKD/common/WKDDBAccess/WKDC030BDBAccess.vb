...
Public Class WKDC030BDBAccess
    ...
    Public Function GetCsvData(processingDate27 As String, processingDate24 As String, conditionDate As String) As DataTable
        ...
        sql.AppendLine("           case when coalesce((select insi_zei + shohi_zei from wao.t_zei where dtnengetu >= @conditionDate order by dtnengetu desc limit 1), 0) = 0 then (select konbini from m_tesuryo)")
        sql.AppendLine("                when coalesce((select insi_zei + shohi_zei from wao.t_zei where dtnengetu >= @conditionDate order by dtnengetu desc limit 1), 0) > skingaku then (select konbini from m_tesuryo)")
