...
Public Class WKDC030BDBAccess
    ...
    Public Function GetCsvData(processingDate27 As String, processingDate25 As String, conditionDate As String) As DataTable
        ...
        sql.AppendLine("select")
        sql.AppendLine("  coalesce(case own.bakyny when '' then null else own.bakyny end,own.bakycd) オーナー№")
        sql.AppendLine(", coalesce(own2.bakome,own.bakome) 校名")
        ...
        sql.AppendLine("left join tbkeiyakushamaster own on (ytd.ownerno = own.bakycd and own.bakome is not null and own.bakyfg = '0')")
